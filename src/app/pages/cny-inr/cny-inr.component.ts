...
@Component({
  selector: 'app-cny-inr',
  templateUrl: './cny-inr.component.html',
  styleUrls: ['./cny-inr.component.scss']
})
export class CNYInrComponent implements OnInit {
...
this.form.month=this.DisplayfinancialYear
  ...
  edit(Rowdata:any)
  {
    ...
    this.form.avgexrate=Rowdata.CnyInr
  }

  getCNY()
  {
    this.common.enableLoader();
    const Url = this.common.Url + 'GetCNYRates'
    ...
    if(this.isUpdate? true : checkData.length==0)

        {
          console.log('yes')
           ...
           this.common.enableLoader();
    const Url = this.common.Url + 'InsUpdCNYRates'
