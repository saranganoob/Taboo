...
@Component({
  selector: 'app-solar-consumption',
  templateUrl: './solar-consumption.component.html',
  styleUrls: ['./solar-consumption.component.scss']
})
export class SolarConsumptionComponent implements OnInit {
      ...
      this.http.post(url,params).subscribe((response:any)=>
      {
        ...
        if(response[status])
        {
          ...
          this.daysData.forEach(item => {
            ...
            console.log(item.C1_value)

            var C1_value_Total=+Number(item.C1_value)

            console.log('C1_value_Total',C1_value_Total)
