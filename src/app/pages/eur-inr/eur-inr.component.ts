...
@Component({
  selector: 'app-eur-inr',
  templateUrl: './eur-inr.component.html',
  styleUrls: ['./eur-inr.component.scss']
})
export class EURInrComponent implements OnInit {
...
this.form.month=this.DisplayfinancialYear
  ...
  edit(Rowdata:any)
  {
    ...
    this.form.avgexrate=Rowdata.EurInr
  }

  getEurrates()
  {
    this.common.enableLoader();
    const Url = this.common.Url + 'GetEURRates'
     ...
     if(this.isUpdate? true : checkData.length==0)

        {
          console.log('yes')
           ...
           this.common.enableLoader();
    const Url = this.common.Url + 'InsUpdEURRates'
